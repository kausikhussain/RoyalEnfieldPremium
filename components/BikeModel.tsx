...
function Wheel({ 
  ...
  const spokes = useMemo(() => {
    const arr = [];
    const count = isAlloy ? 10 : 36; // 36-spoke classic wheels
  ...
  return (
    <group ref={spinRef}>
      ...
      {/* Spokes - Inline mapping to avoid JSX freezing issues */}
...
// Dedicated Exhaust System Subcomponent
function ExhaustSystem({ modelId, wheelBase }: { modelId: string; wheelBase: number }) {
  ...
  // Adventure (Himalayan 450) renders high slung single pipe angled upward
  if (isAdventure) {
    return (
      <group>
...
// Dedicated Seat Subcomponent
function Seat({ modelId, color, wheelBase }: { modelId: string; color: string; wheelBase: number }) {
  const isCruiser = modelId === "supermeteor" || modelId === "meteor";
  const isCafeRacer = modelId === "gt";
  const isAdventure = modelId === "himalayan" || modelId === "bear";
    ...
    return (
      <group position={[-wheelBase * 0.38, 0.65, 0]}>
        ...
        <mesh position={[-0.35, 0.08, 0]} scale={[0.42, 0.26, 0.38]}>
          ...
          <meshPhysicalMaterial color={color} metalness={0.78} roughness={0.1} clearcoat={1.0} clearcoatRoughness={0.08} />
        </mesh>
      </group>
    );
  }
...
export function ProceduralBike({ progress, color, modelId }: BikeProps) {
  ...
  return (
    <group ref={group} scale={1.22} position={bikePosition}>
      ...
      {/* Exhaust Pipes */}
      <ExhaustSystem modelId={modelId} wheelBase={wheelBase} />

      {/* Seat Setup */}
      <Seat modelId={modelId} color={color} wheelBase={wheelBase} />
